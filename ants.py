...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        for i in list:
            i.reduce_armor(self.damage)
        zlist = [z for z in list if z.armor > 0]
        self.place.bees = zlist
        # END Problem 7
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_imposter == True:
            ...
            x = self.place.exit
            while x is not None:
                if x.ant not in self.buffed_ants and x.ant != None:
                    if x.ant.is_container == True:
                        x.ant.damage *= 2
                        if x.ant.contained_ant and x.ant.contained_ant not in self.buffed_ants:
                            x.ant.contained_ant.damage *= 2
                            self.buffed_ants.append(x.ant.contained_ant)
                    else:
                        x.ant.damage *= 2
                    self.buffed_ants.append(x.ant)
                elif x.ant != None and x.ant.is_container:
                    if x.ant.contained_ant and x.ant.contained_ant not in self.buffed_ants:
                        x.ant.contained_ant.damage *= 2
                        self.buffed_ants.append(x.ant.contained_ant)
                self.buffed_ants.append(x.ant)
